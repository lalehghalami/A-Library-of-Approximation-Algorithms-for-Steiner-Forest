...
void readInputFileMPI(string str,double **&adjMatrix,double *&adjLinearArray,int &nVertex,int &nEdges, int &nTerminals,int *& terminals,vector<vector<int > > & sites)
{
    ...
    vector<vector<int> > adj2DVector;
    vector<int> rows(nVertex,INT_MAX);
    for (int i=0; i<nVertex; i++) {
        adj2DVector.push_back(rows);
    }
    int a1,a2,a3;
    myfile>>s;
    while (s=="E") {
        myfile >> a1 >> a2 >> a3;
        adj2DVector[a1-1][a2-1]=a3;
        adj2DVector[a2-1][a1-1]=a3;
        myfile>>s;
        if (s=="END") break;
    }
    
    adjLinearArray = (double *)malloc(sizeof(double)*nVertex*nVertex);
    adjMatrix = (double **)malloc(sizeof(double*)*nVertex);
    for(int i = 0; i < nVertex; i++)
        adjMatrix[i] = &(adjLinearArray[i*nVertex]);
    
    for (int i=0; i<nVertex; i++) {
        for (int j=0; j<nVertex; j++)
            adjMatrix[i][j]=adj2DVector[i][j];
    }
